...
import { formatGenerationMode } from "../ui/viewModels";
...
const modes: ModeConfig[] = [
  {
    mode: "t2i",
    label: MODE_COPY.t2i.short,
    title: MODE_COPY.t2i.title,
    description: MODE_COPY.t2i.description,
    icon: ImageIcon,
    models: imagenModels,
  },
  {
    mode: "t2v",
    label: MODE_COPY.t2v.short,
    title: MODE_COPY.t2v.title,
    description: MODE_COPY.t2v.description,
    icon: FilmIcon,
    models: veoModels,
  },
  {
    mode: "i2v",
    label: MODE_COPY.i2v.short,
    title: MODE_COPY.i2v.title,
    description: MODE_COPY.i2v.description,
    icon: PipelineIcon,
    models: veoModels,
  },
  {
    mode: "pipeline",
    label: MODE_COPY.pipeline.short,
    title: MODE_COPY.pipeline.title,
    description: MODE_COPY.pipeline.description,
    icon: PipelineIcon,
    models: [
      {
...
const defaultPipelineVideoPrompt =
  "카메라가 천천히 전진하며 자전거가 지나가고, 라면집 증기와 네온 반사가 웅덩이에 일렁입니다.";
...
export function GeneratePage() {
  ...
  const heroTitle = heroPrompt || "제목 없는 장면";
  ...
  const isSubmitting = submitMutation.isPending || pipelineMutation.isPending;
  const screenState = enhanceMutation.isPending
    ? "향상 중"
    : isSubmitting
      ? "전송 중"
      : enhanceReview
  ...
  const activeModelLabel =
    mode === "pipeline"
      ? `${modelLabel(pipelineImageModel, imagenModels)} + ${modelLabel(
          pipelineVideoModel,
          veoModels,
        )}`
      : modelLabel(model, activeMode.models);
  const activeAspectLabel =
    mode === "pipeline"
      ? `${pipelineImageAspectRatio} → ${pipelineVideoAspectRatio}`
      : aspectRatio;
  ...
  return (
    <div className="creative-generate">
      <div className="creative-generate__topline">
        <div className="creative-mode-tabs" role="list" aria-label="생성 모드">
          {modes.map((item) => {
            const Icon = item.icon;
            const active = item.mode === mode;
            return (
              <button
                className={`creative-mode-tab${active ? " creative-mode-tab--active" : ""}`}
                key={item.mode}
        ...
        <label className="creative-model-select">
          ...
          <select
            onChange={(event) => {
              if (mode === "pipeline") {
                setPipelineImageModel(event.target.value);
                return;
              }
              setModel(event.target.value);
            }}
            value={mode === "pipeline" ? pipelineImageModel : model}
          >
            {(mode === "pipeline" ? imagenModels : activeMode.models).map((item) => (
      ...
      <section
        className={`creative-stage creative-stage--${mode}${
          isSubmitting || enhanceMutation.isPending ? " creative-stage--busy" : ""
        }`}
        aria-label="생성 미리보기"
      >
        <div className="creative-stage__status">
          <Badge tone={mode === "pipeline" ? "warning" : "info"}>
            <StatusDot tone={isSubmitting || enhanceMutation.isPending ? "pending" : "info"} />
            {screenState}
          </Badge>
        </div>

        <div className="creative-stage__frame">
          <span className="creative-corner creative-corner--tl" />
          <span className="creative-corner creative-corner--tr" />
          <span className="creative-corner creative-corner--bl" />
          <span className="creative-corner creative-corner--br" />
          ...
          {mode === "i2v" && sourceAssetId ? (
            ...
            <div className="creative-stage__copy">
              <div className="creative-stage__eyebrow">
                <span />
                상영 대기
                <span />
              </div>
              <h2>{heroTitle}</h2>
              <p>
                {activeMode.label} · {activeAspectLabel} · {screenState}
              </p>
            </div>
          )}
        </div>
      ...
      >
        {mode === "i2v" && (
          <div
            className={`creative-source-lock${
              sourceAssetId ? " creative-source-lock--connected" : ""
            }`}
          >
            ...
            <span>
              {sourceAssetId
                ? "이전 결과에서 선택한 이미지를 I2V 소스로 사용합니다."
                : "완성된 이미지 결과에서 I2V 시작을 선택하세요."}
            </span>
          </div>
        )}
        ...
        <label className="creative-prompt-field">
          ...
          <textarea
            ...
            placeholder={
              mode === "pipeline"
                ? "Pipeline의 1단계가 먼저 생성할 정지 이미지를 설명하세요."
                : mode === "i2v"
        ...
        {mode === "pipeline" && (
          <label className="creative-prompt-field creative-prompt-field--secondary">
            <span>영상 프롬프트</span>
            <textarea
              onChange={(event) => setPipelineVideoPrompt(event.target.value)}
              placeholder="2단계 I2V 작업에 사용할 카메라 움직임, 피사체 동작, 분위기를 설명하세요."
              value={pipelineVideoPrompt}
            />
          </label>
        )}

        <div className="creative-composer__bottom">
          <div className="creative-compact-controls">
            {mode === "pipeline" && (
              <label className="creative-chip-select">
                <FilmIcon size={12} />
                <select
            ...
            <label className="creative-chip-select">
              ...
              <select
                onChange={(event) => {
                  if (mode === "pipeline") {
                    setPipelineImageAspectRatio(event.target.value);
                    setPipelineVideoAspectRatio(event.target.value);
                    return;
                  }
                  setAspectRatio(event.target.value);
                }}
                value={mode === "pipeline" ? pipelineImageAspectRatio : aspectRatio}
              >
          ...
          <div className="creative-composer__actions">
            <span>{prompt.length}자 · 예상 {mode === "t2i" ? "4초" : `${durationSec + 2}초`}</span>
            <Button
              ...
              variant="secondary"
            >
              {enhanceMutation.isPending ? "향상 중" : "향상"}
            </Button>
            <Button
              disabled={submitDisabled || submitMutation.isPending || pipelineMutation.isPending}
              type="submit"
              variant="primary"
            >
              {submitLabel}
        ...
        {mode === "pipeline" && (
          <div className="creative-inline-note">
            Pipeline은 이미지를 먼저 만들고, 그 결과를 후속 영상의 소스로 사용합니다.
          </div>
        )}

        {usableEnhancementId && (
          <div className="creative-inline-note creative-inline-note--success">
            프롬프트 향상을 수락했습니다. 위 프롬프트가 생성에 사용됩니다.
          </div>
        )}
    ...
    <div className="cinema-source">
      <div className="cinema-source__media">
        {isLoading ? (
          ...
          <img alt={`선택된 I2V 소스 결과 ${imageAsset.id}`} src={imageAsset.url} />
        ) : isError ? (
      ...
      <div className="cinema-screen__content cinema-screen__content--source">
        ...
        <p>
          {imageAsset
            ...
            : "소스 결과는 연결되었지만 이미지 미리보기를 렌더링할 수 없습니다."}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <section className="creative-enhance-drawer">
      <div className="enhance-panel__top">
        <div>
          <div className="section-label">프롬프트 향상</div>
          <h2>향상된 프롬프트 검토</h2>
          <p className="panel-copy">
            Gemini가 편집 가능한 초안을 만들었습니다. 수락하면 요청 구성의 프롬프트로 복사됩니다.
          </p>
        </div>
      ...
      <div className="enhance-meta">
        ...
        <Badge tone="muted">대상 {formatGenerationMode(enhancement.target_mode)}</Badge>
        <Badge tone="muted">
...
function modelLabel(modelId: string, options: ModelOption[]): string {
