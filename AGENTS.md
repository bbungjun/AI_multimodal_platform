...
- Toy/demo shortcut을 기본 선택지로 두지 않습니다. 임시 sleep, 하드코딩된 로컬 경로,
  ...
  설명되거나 제거되어야 합니다.
- Kubernetes 변경은 Pod가 뜨는지만 보지 않습니다. request/limit, readiness/liveness
  또는 startup probe, rollout/rollback 영향, Secret/ConfigMap 경계, Service 노출
  범위, failure mode를 함께 봅니다.
- Terraform/IaC가 가능한 영역은 재현 가능해야 합니다. 불가피한 수동 작업은
  runbook에 목적, 전제조건, 명령, 검증, 되돌리는 방법을 남깁니다.
- 운영 자동화는 안전장치를 포함해야 합니다. GCP 계정/프로젝트 guard, dry-run 또는
  plan 확인, idempotency, 실패 시 중단 조건, 비밀값 미출력을 기본으로 둡니다.
- 관측성은 health endpoint만으로 충분하다고 보지 않습니다. latency, error rate,
  throughput, queue/backlog, worker 상태, provider 실패 코드, quota/rate limit 신호를
  운영자가 확인할 수 있게 발전시킵니다.
- 부하테스트는 통과 결과만 기록하지 않습니다. baseline, stress, failure signal을
  구분하고 p95/p99, HTTP failure rate, dropped iterations, provider 오류를 문서화합니다.
- AI provider 연동은 비용과 실패를 정상 운영 조건으로 다룹니다. 429, 5xx, timeout,
  malformed response, retry/backoff, degraded mode, 사용자에게 보이는 에러 메시지를
  테스트 가능한 경계로 구현합니다.
- 보안은 나중 문제가 아닙니다. Workload Identity와 최소 권한을 우선하고, Secret
  payload, ADC, service-account JSON, API key/private key는 읽거나 출력하거나 커밋하지
  않습니다.
- CI/CD는 "빌드 성공"을 넘어 배포 가능성을 확인해야 합니다. 이미지 태그, migration
  또는 schema 영향, rollback 방법, 환경별 설정 차이를 PR 또는 runbook에 남깁니다.
- 성능/비용/쿼터 변경은 숫자로 다룹니다. replica, concurrency, rate limit, resource
  request, autoscaling threshold는 이유와 검증 결과가 있어야 합니다.
- 구현한 증거와 계획을 섞지 않습니다. 실제로 배포/검증한 것은 결과와 명령을 남기고,
  GPU node pool, NVIDIA device plugin, 분산 학습 운영처럼 아직 구현 전인 항목은
  명확히 다음 단계로 표시합니다.
...
- `docs/current-work.md`에는 현재 상태와 다음 작업을 짧게 갱신하고, 장기간 보존할
  ...
  `docs/portfolio/issue-번호-짧은-설명.md`에 상세 기록을 남깁니다.
- 상세 기록에는 최소한 다음 내용을 포함합니다.
  1. **배경과 문제:** 어떤 사용자/운영 문제가 있었고, 기대 동작과 실제 동작이 어떻게
     달랐는지 설명합니다.
  2. **관측과 원인 분석:** 로그, metric, 상태, 재현 절차와 검토한 가설을 기록합니다.
     실패한 접근이나 No-Go 판단도 숨기거나 성공한 것처럼 다시 쓰지 않습니다.
  3. **해결 방법과 판단 근거:** 선택한 구현, 검토한 대안, trade-off, 안전장치와
     rollback 방법을 설명합니다.
  4. **검증:** 실행한 명령, 환경과 전제조건, pass/fail 결과를 남깁니다. 부하·성능·비용
     작업은 가능한 경우 before/after, p95/p99, error rate, throughput, replica/node 수,
     요청 수와 비용 범위를 숫자로 기록합니다.
  5. **결과와 영향:** 사용자가 확인할 수 있는 변화와 운영상 개선을 정리하고, 실제로
     배포/실행해 확인한 범위를 명시합니다.
  6. **남은 위험과 다음 단계:** 미검증 항목, 알려진 한계, 후속 Issue를 구분해 남깁니다.
- 스크린샷, 대시보드, Terraform plan, k6 요약, CI 결과, 장애 타임라인처럼 판단을
  뒷받침하는 artifact가 있으면 비밀정보를 제거한 뒤 repo-relative 경로 또는 안정적인
  링크로 연결합니다. 긴 원본 로그를 그대로 커밋하기보다 핵심 신호와 재현 가능한
  명령을 보존합니다.
- `Implemented`, `Live Verified`, `Planned`를 명확히 구분합니다. 코드가 존재한다는
  사실을 실제 클라우드/GPU 환경에서 검증한 결과처럼 표현하지 않습니다.
- Secret payload, credential, prompt 원문, provider raw response, 개인 PC absolute path,
  계정 식별 정보는 포트폴리오 문서와 증거 artifact에 기록하지 않습니다.
- 작업을 마치기 전 코드, 테스트, env, runbook, 포트폴리오 기록이 서로 일치하는지
  확인합니다. 최종 보고에는 어떤 문서를 갱신했고 어떤 증거를 남겼는지 포함합니다.
