...
## 사용자 입회 Chrome 화면 시연 — 2026-09-14

사용자가 `@Chrome`을 선택한 뒤 브라우저 확장 연결의 `cua_repl`로 실제 보이는 탭을
조작했다. 앞선 Windows native Computer Use 시도는 URL 식별 실패로 중단됐으며,
이번 성공이 그 native 연결 문제의 해결을 의미하지 않는다.

- 에이전트가 접근성 정보에서 로그인/입력/향상/수락/생성/기록 행을 확인하고 클릭했다.
  원본 입력과 초안 편집은 키보드로 수행했다. 제품 health에서 mock_provider를 확인했다.
- 편집 후 메인 입력의 길이248자와 추가 문장의 포함을 DOM으로 확인했다. 실제 화면의
  작업 요청 요약에도 같은 편집 내용과 `edited_after_enhancement=true`가 표시됐다.
- 이미지 완료 화면에서 최초 이미지의 로딩과 실제640×360 크기를 DOM으로 확인하고,
  화면에 표시된 mock 패턴을 직접 관측했다. 생성된 결과는 실제 Imagen 품질 증거가 아니다.
- 새로고침 뒤 동일 Job/asset이 접근성 정보에 표시됐다. 기록 목록의1개 작업 행을
  클릭해 같은 상세 URL과 동일한 결과 이미지를 화면에서 다시 확인했다.
- 새로고침 후 보조 Playwright DOM locator 검사는3초 timeout이 두 차례 발생했다.
  화면/접근성 조작은 계속 작동하여 이를 사용했다. 재조회 뒤 pixel/hash/자연 크기를
  기계적으로 재확인했다고 주장하지 않는다. 확장 연결의 Console 조회에는 React Router
  future-flag 경고6건만 반환됐다. 전체 Network를 별도 수집한 실행은 아니다.
- 요청값의 byte-level 비교와 파일 hash를 포함한 앞선 MCP receipt와 이번 화면 시연을
  합쳐 하나의 무결점 자동 실행으로 표시하지 않는다. 시연의 근거는 대화의 실제 도구
  결과와 화면이다. 프롬프트/계정/작업 ID가 보이는 스크린샷을 저장소에 커밋하지 않았다.
- 관측 당시 repo HEAD는 `cd61c3b`; 이번 시연용 source hash는 별도 수집하지 않았다.
  변경한 것은 ignored 임시 실행 보조 파일과 이 문서뿐이며 제품 코드는 바꾸지 않았다.
- 원래 native 시연용 보조 관측기는 다른 임시 Chrome 프로필에 붙어 있었으므로 그
  image 판정을 확장 탭의 QA 결과로 사용하지 않았다. 종료 명령 후 보조 Chrome과
  owned Compose cleanup0,18156 listener 없음, 확장 QA 탭 닫힘을 확인했다.

확인된 범위는 사용자 입회하에 실행한 로그인→향상·편집·수락→이미지 생성→새로고침→
기록 재조회 화면 흐름1개다. 새 제품 수정이나 전체 회귀 suite 실행은 포함하지 않는다.
