...
| Auth emergency | Guarded emergency Session revocation | In Progress | [Issue99](https://github.com/bbungjun/AI_multimodal_platform/issues/99), [accepted spec](emergency-session-revocation-spec.md); branch `codex/issue-99-emergency-session-revocation` from G9A squash `2565a7a` | Migration0; disable start/callback admission before guarded preview/apply. Mock proof only; live drill remains separate |
