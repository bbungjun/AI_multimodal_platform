...
- On 2026-09-02, the initiative and accepted G1 specification merged through
  ...
  the one-module and one-migration limits remain unchanged. No migration,
  reset, product-code change, or database mutation has occurred yet.
  ...
  It contains eight sequential Todos and four final verifiers. Todo 1 fixed the
  exact 22-path map and added the Alembic/schema-control contracts; the focused
  red run produced 10 expected failures for missing G1 behavior with no import
  or fixture error. Todo 2 Alembic packaging is next.
