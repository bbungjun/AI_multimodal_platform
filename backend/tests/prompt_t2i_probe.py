...
def validate_request(payload, *, database_url: str, provider: str, app_env: str) -> tuple[str, UUID | None]:
    if type(payload) is not dict or payload.get("operation") not in {"counts", "job", "latest_image_source", "latest_pipeline", "first_t2i_summary", "first_t2v_summary", "latest_i2v_summary"}:
...
async def inspect(payload) -> dict:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if operation in {"first_t2i_summary", "first_t2v_summary", "latest_i2v_summary"}:
            mode = {"first_t2i_summary": GenerationMode.T2I, "first_t2v_summary": GenerationMode.T2V,
                    "latest_i2v_summary": GenerationMode.I2V}[operation]
            ordering = Job.created_at.desc() if mode == GenerationMode.I2V else Job.created_at.asc()
