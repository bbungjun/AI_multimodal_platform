...
def load():
    spec = importlib.util.spec_from_file_location("verify_accounting",ROOT/"scripts/verify_credit_accounting.py")
...
def payload(m):
    return dict(groups=dict.fromkeys(m.GROUPS,True),checks=160,races=8,complete=True)


@pytest.mark.parametrize("change",[dict(complete=False),dict(races=7),dict(races=True),dict(checks=159),dict(checks=True),
...
@pytest.mark.parametrize("value",["default","creativeops-login-preview","accounting-verify-123","accounting-verify-123456ABCDEF","accounting-verify-../"])
...
@pytest.mark.parametrize("status",[" M backend/app/credit_accounting.py","?? arbitrary.py","R  docs/x -> backend/x.py"])
...
def test_only_db_and_migrate_fixed_config(tmp_path,monkeypatch):
    ...
    assert selected["services"]["migrate"]["environment"]["ACCOUNTING_PROOF_PROJECT"] == r.project
