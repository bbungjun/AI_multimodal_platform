...
GROUPS = ("input_quote", "allocation", "reserve_replay", "settlement", "release", "transaction", "integrity", "concurrency")
WORK_SECONDS, CLEANUP_SECONDS = 300, 90
PROOF = ROOT / "backend/tests/credit_accounting_support.py"
EVIDENCE = ROOT / ".omo/evidence/issue-121"
LABEL = "creativeops.accounting-proof"
...
def command(args, *, env, timeout, input=None):
    ...
    if result.returncode:
        ...
        match = re.fullmatch(r"accounting_proof_failed:(guard|input_quote|allocation|reserve_replay|settlement|release|transaction|integrity|concurrency|done)\s*",result.stdout)
...
def parse_proof(output):
    ...
    if (not isinstance(value,dict) or set(value) != {"groups","races","checks","complete"}
            ...
            or type(value["checks"]) is not int or value["checks"] < 160):
...
class Runtime:
    def __init__(self, env_file, *, run=command):
        ...
        self.env = safe_env()
        self.run = run
        self.project = validate_project("accounting-verify-"+uuid4().hex[:12])
    ...
    def configure(self,directory):
        ...
        migrate["environment"] = dict(DATABASE_URL=f"postgresql+asyncpg://credit:local_mock_only@db:5432/{dbname}",
            ...
            AUTH_GOOGLE_CLIENT_SECRET="",AUTH_GOOGLE_REDIRECT_URI="",ACCOUNTING_PROOF_PROJECT=self.project)
...
def verify(env_file,*,run=command):
    ...
    with tempfile.TemporaryDirectory(prefix="accounting-proof-") as directory:
        ...
        try:
            ...
            print("phase=accounting",flush=True)
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Fixed local mock Credit accounting proof")
