...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Reenqueue resumable Veo polling jobs without reading .env.",
    )
    ...
    try:
        ...
        print(f"REENQUEUE POLLING FAILED: {exc}", file=sys.stderr)
        return 1
    except KeyboardInterrupt:
        print("REENQUEUE POLLING FAILED: interrupted", file=sys.stderr)
        return 130

    print(
        "REENQUEUE POLLING COMPLETE "
...
async def _run(*, limit: int):
    ...
    from app.services.jobs.repair import reenqueue_resumable_polling_jobs

    return await reenqueue_resumable_polling_jobs(limit=limit)
