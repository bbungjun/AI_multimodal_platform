...
## Follow-up: state history payload detail contract restored

2026-05-28 후속 작업에서 `state_history` entry가 Job Detail / Pipeline timeline이
기대하는 payload shape를 일관되게 유지하도록 테스트와 구현을 보강했습니다. 실제
Vertex/Gemini/Imagen/Veo 호출은 수행하지 않았고 state machine, fake runner, fake
handler session만 사용했습니다.

복구 근거:

- `README.md`는 runner와 handler의 상태 변경이 `transition(...)`을 통해 검증되고
  `state_history`에 기록되며, Job Detail은 `state_history` 기반 timeline을 표시한다고
  설명합니다.
- `memories/architecture.md`는 `state_history JSONB` shape를
  `[{state, at, detail}]`로 정리하고, `error JSONB`는
  `{code, message, retry_count, last_attempt_at}` 형식이라고 기록합니다.
- `frontend/src/api/types.ts`는 `StateHistoryEntry`를 `state`, `at`,
  `detail: JsonObject | null`로 사용합니다.

복구한 계약:

- `transition(...)`은 `detail`이 없는 상태 전이도 `detail: null`로 기록합니다.
- runner handler failure와 orphan sweep failure의 `state_history[-1].at`은
  `error.last_attempt_at`과 같은 timestamp를 사용합니다.
- failure timeline entry는 `{state, at, detail}` shape를 유지하며, `detail`에는
  public error code 또는 `runner_startup_sweep` reason이 남습니다.
- completed 같은 terminal timeline entry도 `detail: null`을 포함하므로 frontend timeline
  payload shape가 안정적입니다.

검증 결과:

- RED: 기존 구현에서 `detail` 없는 completed transition은 `state/at`만 저장해
  `test_transition_records_null_detail_when_no_detail_is_supplied`와 T2I completed
  history payload shape 검증이 실패함을 확인했습니다.
- GREEN: `transition(...)`이 항상 `detail` 키를 기록하도록 수정한 뒤 같은 좁은 테스트
  5개가 통과했습니다.
- `AI_PROVIDER=mock python -m pytest tests/test_state_machine.py tests/test_job_runner.py tests/test_job_handlers.py -q`
  -> `29 passed`
